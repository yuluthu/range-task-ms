...
export default function Product({productInfo}: {productInfo: any}) {
  return (
    <div key={productInfo.id} className="col-span-1cursor-pointer border-1 border-mist-200 text-center" >
      <div className="grid grid-rows-14">
        <Image className="row-span-7 p-3" src={'/products/'+productInfo.img_id+'.jpg'} alt={productInfo.name} width={200} height={200} />
        ...
        <div className="row-span-1 text-red-500">
          {productInfo['was_price'] ? <span className="line-through">Was {new Intl.NumberFormat("en-GB", { style: "currency", currency: "GBP" }).format(productInfo['was_price']/100)}</span> : null}
        </div>
